...
async fn main() {
    ...
    let mut _locations_list = resource_manager::get_byte_sequence_locations_list();

    // Returns HTML as string
    let website_one = resource_manager::get_website(website_url_one).await.unwrap();
    let website_two = resource_manager::get_website(website_url_two).await.unwrap();

    website_compiler::compute_matching_sequences();
