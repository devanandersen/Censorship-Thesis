...
pub fn final_stats_and_proportions(candidate_website: String, helper_websites: Vec<String>, sequence_length: usize) {
    let candidate_length = candidate_website.chars().count();
    let mut proportion_vec = vec![0; sequence_length];
    let comment_regex = Regex::new(r"<!--(.*?)-->").unwrap();
    let mut increase_in_space: f64 = 0.0;
    let mut total_sequences_mapped = 0;
    let mut unmapped_helper_length = 0;
    let mut mapped_helper_length = 0;
    for helper_website in helper_websites.iter() {
        mapped_helper_length += helper_website.chars().count();
        ...
        total_sequences_mapped += comment_for_compiling.len();
        let unmapped_helper_website = comment_regex.replace_all(&helper_website, "").trim().to_string();
        unmapped_helper_length += unmapped_helper_website.chars().count();

        increase_in_space += ((mapped_helper_length as f64 - unmapped_helper_length as f64) / unmapped_helper_length as f64) * 100 as f64;
